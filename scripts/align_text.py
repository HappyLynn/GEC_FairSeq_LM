...
def getAutoAlignedEdits(orig, cor, spacy, args_lev, args_merge):
	...
	if args_lev: alignments = DL.WagnerFischer(orig_toks, cor_toks, orig, cor, substitution=levSubstitution, transposition=levTransposition)
	...
	if args_merge == "rules": edits = get_edits(orig, cor, get_opcodes(alignment))
	elif args_merge == "all-split": edits = get_edits_split(get_opcodes(alignment))
	elif args_merge == "all-merge": edits = get_edits_group_all(get_opcodes(alignment))
	elif args_merge == "all-equal": edits = get_edits_group_type(get_opcodes(alignment))
	proc_edits = []
	for edit in edits:
		orig_start = edit[1]
		orig_end = edit[2]
